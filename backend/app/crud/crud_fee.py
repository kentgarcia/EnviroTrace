...
class CRUDAirQualityFee(CRUDBase[AirQualityFee, AirQualityFeeCreate, AirQualityFeeUpdate]):
    def get_by_fee_id(self, db: Session, *, fee_id: int) -> Optional[AirQualityFee]:
    ...
    def get_by_offense_level(self, db: Session, *, offense_level: int) -> List[AirQualityFee]:
        return db.query(AirQualityFee).filter(AirQualityFee.offense_level == offense_level).all()
    ...
    def remove_sync(self, db: Session, *, fee_id: int) -> Optional[AirQualityFee]:
        """Synchronous version of remove for use with sync sessions"""
        obj = db.query(AirQualityFee).filter(AirQualityFee.fee_id == fee_id).first()
        if obj:
            db.delete(obj)
            db.commit()
        return obj
