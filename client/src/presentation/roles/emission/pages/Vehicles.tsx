          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#F9FBFC]">
            ...
            <Card className="mt-6">
              <div className="p-6">
                {/* Status Tabs */}
                {(() => {
                  const allCount = filteredVehicles.length;
                  const passedCount = filteredVehicles.filter(
                    (v) => v.latestTestResult === true
                  ).length;
                  const failedCount = filteredVehicles.filter(
                    (v) => v.latestTestResult === false
                  ).length;
                  const untestedCount = filteredVehicles.filter(
                    (v) => v.latestTestResult == null
                  ).length;
                  const tabList = [
