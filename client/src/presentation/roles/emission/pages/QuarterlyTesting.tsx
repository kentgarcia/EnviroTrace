...
import { ScheduleTable } from "@/presentation/roles/emission/components/quarterly/ScheduleTable";
...
export default function QuarterlyTesting() {
  ...
  const {
    ...
    handleYearChange,
    handleQuarterChange,
    schedules, // Add schedules from the hook
    refetchSchedules,
    refetchTests,
    addSchedule,
    editSchedule,
  ...
  // Get the schedules and tests data  // Get emission test data
  ...
  const selectedSchedule =
    schedules?.find((s) => s.id === selectedScheduleId) || null;// Simple filter logic for emission tests
  ...
  const handleExportTestResults = () => {
    ...
    emissionTests.forEach((test) => {
      ...
      const result = test.result ? "Passed" : "Failed"; csvContent += `"${test.vehicle?.plate_number || ""}","${test.vehicle?.driver_name || ""
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 bg-[#F9FBFC]">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              ...
              {/* Filters (right) */}
              <div className="flex flex-wrap gap-2 items-center justify-end">              <Button
                variant="outline"
                className="min-w-[120px] justify-between bg-white pr-8"
                onClick={() => setResult(result === "" ? "passed" : result === "passed" ? "failed" : result === "failed" ? "untested" : "")}
              >
                {result
                  ? result === "passed"
                    ? "Passed"
