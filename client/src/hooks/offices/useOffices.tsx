...
export function useOffices() {
  ...
  const officeData = useMemo(() => {
    ...
    if (filters.year) {
      vehicles = vehicles.filter(
        (v) => !v.latestTestYear || v.latestTestYear === filters.year
      );
    }

    if (filters.quarter) {
      vehicles = vehicles.filter(
        (v) => !v.latestTestQuarter || v.latestTestQuarter === filters.quarter
      );
    }
  ...
  const summaryStats = useMemo(() => {
    ...
    const totalVehicles = officeData.reduce(
      (sum, o) => sum + o.vehicleCount,
      0
    );
    const totalCompliant = officeData.reduce(
      (sum, o) => sum + (o.complianceRate >= 80 ? 1 : 0),
      0
    );
    const overallComplianceRate =
      totalOffices > 0 ? Math.round((totalCompliant / totalOffices) * 100) : 0;
  ...
  const handleFilterChange = useCallback(
    (filterUpdates: Partial<OfficesFilter>) => {
