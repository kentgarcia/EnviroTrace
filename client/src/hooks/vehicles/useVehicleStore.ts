  ...
  persist(
    (set, get) => ({
      ...
      actions: {
        ...
        addPendingVehicle: (vehicle) => set(state => ({
          pendingVehicles: [...state.pendingVehicles, { ...vehicle, id: `pending-${Date.now()}` }]
        })),
        addPendingUpdate: (id, vehicle) => set(state => ({
          pendingUpdates: { ...state.pendingUpdates, [id]: vehicle }
        })),
        addPendingDelete: (id) => set(state => ({
          pendingDeletes: [...state.pendingDeletes, id]
        })),
        removePendingVehicle: (id) => set(state => ({
          pendingVehicles: state.pendingVehicles.filter(v => v.id !== id)
        })),
        removePendingUpdate: (id) => set(state => {
          const { [id]: _, ...restUpdates } = state.pendingUpdates;
          return { pendingUpdates: restUpdates };
        }),
        removePendingDelete: (id) => set(state => ({
          pendingDeletes: state.pendingDeletes.filter(deleteId => deleteId !== id)
        })),
