...
export function ComplianceSummaryCard({
    ...
    const totalTested = officeData.reduce((sum, office) => sum + office.testedCount, 0);
    const totalPassed = officeData.reduce((sum, office) => sum + office.passedCount, 0);
    const totalOffices = officeData.length;

    // Calculate overall compliance rate
    const overallComplianceRate = totalVehicles ? Math.round((totalPassed / totalVehicles) * 100) : 0;
    ...
    const cards = [
        {
            ...
            iconClassName: overallComplianceRate >= 80 ? "text-green-500" : "text-red-500",
            difference: complianceDiff !== 0 ? (
                <div className={`flex items-center text-xs ${isImproving ? "text-green-500" : "text-red-500"}`}>
                    {isImproving ? <ArrowUpRight className="h-4 w-4 mr-1" /> : <ArrowDownRight className="h-4 w-4 mr-1" />}
            ...
            {cards.map((card, index) => (
                <Card key={index}>
                    ...
                    <CardContent>
                        ...
                        {card.difference && <div className="text-xs text-muted-foreground">{card.difference}</div>}
                    </CardContent>
                </Card>
            ))}
        </div>
